...
class Player:
    ...
    def max(self, opponent):
        """
        Maximizes score

        parameter:
            opponent - the opposing symbol

        returns:
            max_score - the maximum possible score
            row - the row for the maximum score
            col - the column for the maximum score
        """
        max_score = -2
        row = None
        col = None
        ...
        for i in range(len(table)):
            for j in range(len(table)):
                if table[i][j] == "_":
                    table[i][j] = self.symbol
                    score, min_i, min_j = self.min(opponent)
    ...
    def min(self, opponent):
        ...
        returns:
            ...
            row - the row for the minimum score
            col - the column for the minimum score
        """
        min_score = 2
        row = None
        col = None
        ...
        for i in range(len(table)):
            for j in range(len(table)):
                if table[i][j] == "_":
                    table[i][j] = opponent.symbol
                    score, max_i, max_j = self.max(opponent)
    ...
    def hard_move(self, opponent):
        """
        Completes one move of hard difficulty  based of off scores for the computer and prints the result
        ...
        score, row, col = 0, 0, 0
        if self.symbol == "X":
            score, row, col = opponent.min(self)
        elif self.symbol == "O":
            score, row, col = self.max(opponent)
        table[row][col] = self.symbol
        print(f"Making move level \"{self.difficulty}\"")
        print_table()
        check_win()
...
def winner_symbol():
    """
    Determines if a symbol wins and returns results

    returns:
        symbol - the winning symbol
        draw - if no symbol wins
        None - if there is still empty spots on the table
    """
    symbol = ""
    for i in range(len(table)):
        if table[i][0] == table[i][1] and table[i][1] == table[i][2] and table[i][0] != "_":
...
def play_game():
    ...
    while win == 0 and draw == 0:
        ...
        elif player_1.difficulty == "hard":
            player_1.hard_move(player_2)
        ...
        if win == 0 and draw == 0:
            ...
            elif player_2.difficulty == "hard":
                player_2.hard_move(player_1)
    win = 0
    draw = 0
    table = [['_', '_', '_'], ['_', '_', '_'], ['_', '_', '_']]
